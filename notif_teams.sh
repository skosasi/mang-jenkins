...
## example command
# ./notisJenkins.sh webhookendpoint appname servicename branch status buildnumber commitauthor commitmessage coverage


#variable date
currDate=$(date '+%Y-%m-%d %H:%M:%S')
COVERAGE_NOT_AVAIL='CODE COVERAGE NOT AVAILABLE'
...
commitauthor=$7
commitmessage=$8
coverage="${9:-$COVERAGE_NOT_AVAIL}" # number percentage


#input validation
[[ -z "$endpoint" ]] && { echo "endpoint is empty" ; exit 1; }
[[ -z "$appname" ]] && { echo "appname is empty" ; exit 1; }
[[ -z "$servicename" ]] && { echo "servicename is empty" ; exit 1; }
[[ -z "$branch" ]] && { echo "branch is empty" ; exit 1; }
[[ -z "$status" ]] && { echo "status is empty" ; exit 1; }
[[ -z "$buildnumber" ]] && { echo "buildnumber is empty" ; exit 1; }
[[ -z "$commitauthor" ]] && { echo "commitauthor is empty" ; exit 1; }
[[ -z "$commitmessage" ]] && { echo "commitmessage is empty" ; exit 1; }
...
##color coverage
coveragecolor="🟥"
if [ "$coverage" == "$COVERAGE_NOT_AVAIL" ]; then
    coveragecolor="🟥"
...
listEnv_develop='Development'
listEnv_release='Staging'
listEnv_master='Production'

envVarName="listEnv_"$branch
    ...
    "potentialAction": [
        {
            ...
            "targets": [
                {
                    "os": "default",
                    "uri": "https://jenkins.playcourt.id/job/${servicename}/job/${branch}/"
                }
            ]
        },
        {
